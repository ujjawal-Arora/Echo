...
function SearchCard({ avatar, name, message, date ,count,keys,conversationId}:{avatar:string|null,name:string,message:string,date:ReactNode,count:number,keys:number,conversationId:string}) {
  ...
  return (
    <div className="flex items-center p-4 border-b shadow dark:border-gray-600 border-gray-300 gap-4 rounded cursor-pointer font-mono " onClick={handleclick}>
     <div>
      ...
      <div className="flex-1">
        <div className="flex justify-between items-start">
          ...
          <div className="flex flex-col items-end text-gray-500 text-xs dark:text-gray-200">
            <span>{date}</span>
            <div className={`text-white bg-[#DB1A5A] rounded-full flex mt-2 justify-center place-items-center ${count<=9?"pr-[0.5rem] pl-[0.5rem] p-[0.1rem]":count<=99?"pr-[0.3rem] pl-[0.3rem] p-[0.2rem]":"pr-[0.3rem] pl-[0.3rem] p-[0.1rem]"}`}>
